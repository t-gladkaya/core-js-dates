...
function getNextFriday(date) {
  ...
  const currentDayOfTheWeek = nextFridayDate.getDay();
  const FRIDAY = 5;

  let daysToAdd = FRIDAY - currentDayOfTheWeek;

  if (daysToAdd <= 0) {
    daysToAdd += 7;
  }

  nextFridayDate.setDate(nextFridayDate.getDate() + daysToAdd);
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  return Math.floor(Math.abs(end - start) / MS_PER_DAY) + 1;
}
...
function formatDate(date) {
  ...
  const formattedDate = givenDate.toLocaleDateString('en-US', dayOptions);
 ...
 * The first week of the year is defined according to ISO8601. - первый четверг месяца, первый чт
...
function getWeekNumberByDate(date) {
  ...
  const diffInDays = Math.floor((d - yearStart) / oneDayMs) + 1;
